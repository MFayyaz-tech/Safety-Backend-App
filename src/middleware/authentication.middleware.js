...
const adminUserService = require("../resources/admin_user/adminUserService");
...
exports.authUser = expressAsyncHandler(async (req, res, next) => {
  try {
    ...
    jwt.verify(token, process.env.JWTKEY, async (error, decodedUser) => {
      ...
      const adminData = await adminUserService.getOne(decodedUser.userId);
      if (adminData) {
        req.user = adminData;
        return next();
      }
...
exports.authAdmin = expressAsyncHandler(async (req, res, next) => {
  try {
    ...
    jwt.verify(token, process.env.JWTKEY, async (error, decodedUser) => {
      ...
      const userData = await adminUserService.getOne(decodedUser?.userId);
      if (!userData) {
        return await sendResponse(
          res,
          responseStatusCodes.UNAUTHORIZED,
